...
object LowFarePlusRSObjectParser {
  ...
  def createPricingInfoWSResponse(airItineraryPricingInfo: AirItineraryPricingInfo): PricingInfoWSResponse = {
    ...
    val baseFare: Double = airItineraryPricingInfo.getItinTotalFare.getBaseFare.getAmount / 100
    var tax: Double = .0
    airItineraryPricingInfo.getItinTotalFare.getTaxes.getTax.foreach(a => tax += a.getAmount)
    tax /= 100
    val total: Double = airItineraryPricingInfo.getItinTotalFare.getTotalFare.getAmount / 100
    ...
    pricingInfoWSResponse.setPassengerFare(airItineraryPricingInfo.getPTCFareBreakdowns.getPTCFareBreakdown.map(ptcFareBreakDown => FareBreakDown(
      ...
      passengerFare = entity.PassengerFare(baseFare = BigDecimal(ptcFareBreakDown.getPassengerFare.getBaseFare.getAmount / 100), totalTax = BigDecimal(ptcFareBreakDown.getPassengerFare.getTaxes.getTax.map(_.getAmount.doubleValue()).sum / 100), totalFare = BigDecimal(ptcFareBreakDown.getPassengerFare.getTotalFare.getAmount / 100))
    )).toList)
  ...
  def createOriginDestinationWSResponse(originDestinationOption: OriginDestinationOption): OriginDestinationWSResponse = {
    ...
    val flightSegments = originDestinationOption.getFlightSegment.asScala.toList.map(createFlightSegmentWSResponse(_))
    ...
    originDestinationWSResponse.setFlightSegmentWSResponses(flightSegments)
    originDestinationWSResponse.setDuration(flightSegments.head.getDurationTripXML)
  ...
  def createFlightSegmentWSResponse(flightSegment: FlightSegment)(implicit airlines: Map[String, Airlines], airports: Map[String, Airports]): FlightSegmentWSResponse = {
    ...
    val departureAirport = airports.getOrElse(flightSegment.getDepartureAirport.getLocationCode, Airports())
    val arrivingAirport = airports.getOrElse(flightSegment.getArrivalAirport.getLocationCode, Airports())
    ...
    val cabinContent = flightSegment.getTPAExtensions.getContent.head.asInstanceOf[JAXBElement[CabinType]]
    val cabinType = cabinContent.getValue
    flightSegmentWSResponse.setCabin(cabinType.getCabin.value())
    val durationContent = flightSegment.getTPAExtensions.getContent.get(1).asInstanceOf[JAXBElement[String]]

//    val duration = durationContent.getValue.split(":")
...
//
    ...
    flightSegmentWSResponse.setDurationTripXML(durationContent.getValue)
    flightSegmentWSResponse.setDuration(s"""${hours}H:${minutes}M""")
