...
/**
  ...
  */
object EPowerGetFareRuleInvoker extends Invoker[GetFlightRulesResponse] with EPowerHeader {


  override def invoke(value: AnyRef): GetFlightRulesResponse = {
    val checkETicket = value.asInstanceOf[GetFlightRules]
    ...
    var checkETicketResponse: GetFlightRulesResponse = null
    ...
    val unMarshaller: Unmarshaller = JAXBContext.newInstance(classOf[GetFlightRulesResponse]).createUnmarshaller
    checkETicketResponse = unMarshaller.unmarshal(responseBody.extractContentAsDocument).asInstanceOf[GetFlightRulesResponse]
    checkETicketResponse
  }
  override def invokeAsync(value: AnyRef)(implicit executionContext: ExecutionContext): Future[GetFlightRulesResponse] = Future {
    invoke(value)
  }
