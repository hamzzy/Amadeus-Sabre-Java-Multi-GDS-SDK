...
/**
  ...
  */
object EPowerReadPNRInvoker extends Invoker[GetPNRResponse] with EPowerHeader {

  override def invokeAsync(value: AnyRef)(implicit executionContext: ExecutionContext): Future[GetPNRResponse] = Future(invoke(value))

  override def invoke(value: AnyRef): GetPNRResponse = {
    val getPnr = value.asInstanceOf[GetPNR]
    ...
    var getPnrResponse: GetPNRResponse = null
    val requestXml = createSOAP(StringUtil.classToXmlString(getPnr).replaceAllLiterally("<?xml version=\"1.0\" encoding=\"UTF-8\" standalone=\"yes\"?>", ""))
//    println(requestXml)
    ...
    val unMarshaller: Unmarshaller = JAXBContext.newInstance(classOf[GetPNRResponse]).createUnmarshaller
    getPnrResponse = unMarshaller.unmarshal(responseBody.extractContentAsDocument).asInstanceOf[GetPNRResponse]
    getPnrResponse
  }
